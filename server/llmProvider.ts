...
export interface LLMConfig {
  provider: "kimi" | "openai" | "gemini" | "anthropic" | "litellm" | "qwen";
  apiKey: string;
}
...
function resolveEndpoint(llmConfig?: LLMConfig | null): { baseUrl: string; apiKey: string; model: string } {
  if (llmConfig) {
    switch (llmConfig.provider) {
      ...
      case "qwen":
        return { baseUrl: "https://dashscope-intl.aliyuncs.com/compatible-mode/v1", apiKey: llmConfig.apiKey, model: "qwen3.5-122b-a10b" };
      case "litellm":
        return {
          baseUrl: process.env.LITELLM_BASEURL || "https://litellm.koboi2026.biz.id/v1",
  ...
  const LITELLM  = process.env.LITELLM_API;
  const LITELLM_BASE = process.env.LITELLM_BASEURL || "https://litellm.koboi2026.biz.id/v1";
  const OPENAI   = process.env.OPENAI_API_KEY;
  const ANTHROPIC = process.env.ANTHROPIC_API;
  const KIMI     = process.env.KIMI_API;
  const QWEN     = process.env.QWEN_API;
  ...
  if (QWEN)      return { baseUrl: "https://dashscope-intl.aliyuncs.com/compatible-mode/v1", apiKey: QWEN, model: "qwen3.5-122b-a10b" };
  if (ANTHROPIC) return { baseUrl: "https://api.anthropic.com/v1", apiKey: ANTHROPIC, model: "claude-haiku-4-5" };

  throw new Error("No LLM provider configured. Set LITELLM_API, OPENAI_API_KEY, KIMI_API, or ANTHROPIC_API.");
}
