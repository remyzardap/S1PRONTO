...
- **Chat (S1)**: Streaming SSE chat with automatic intelligent agent routing across LiteLLM, Claude, Sonar, and Kimi
...
The chat system (`server/routers/s1Router.ts`) classifies every incoming message and routes it to the best agent:
- **LiteLLM** (GPT-5.2 via proxy) — default/general purpose
- **Claude** (Anthropic) — writing/reasoning
- **Sonar** (Perplexity) — web/real-time queries
- **Kimi** (Moonshot) — code/technical/long-context

Fallback order: `preferred → litellm → claude → kimi → sonar`. If any agent's key is missing or fails, it automatically tries the next.
...
**Key tables**: `users`, `identities`, `skills`, `memories`, `connections`, `chat_messages`, `files`, `api_keys`, `receipts`, `tasks`, `procurementRequests`, `messageLogs`, `payments`, `businesses`, `auditLogs`, `betaInviteCodes`, `passwordResetTokens`, `emailVerificationTokens`, `blocks`
