...
// Agent model → color mapping
const MODEL_COLORS: Record<string, { label: string; color: string }> = {
  sonar: { label: "Sonar", color: "#2dd4bf" },
  "sonar-pro": { label: "Sonar Pro", color: "#2dd4bf" },
  kimi: { label: "Kimi", color: "#f59e0b" },
  "moonshot-v1-128k": { label: "Kimi", color: "#f59e0b" },
  claude: { label: "Claude", color: "#f97316" },
  "claude-haiku": { label: "Claude Haiku", color: "#f97316" },
  "claude-sonnet": { label: "Claude Sonnet", color: "#f97316" },
  gemini: { label: "Gemini", color: "#a78bfa" },
  "gemini-2.0-flash": { label: "Gemini Flash", color: "#a78bfa" },
};

function getModelMeta(model?: string) {
  if (!model) return null;
  const key = Object.keys(MODEL_COLORS).find((k) => model.toLowerCase().includes(k));
  return key ? MODEL_COLORS[key] : { label: model, color: "rgba(242,242,242,0.3)" };
}
